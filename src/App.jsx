...
const OCCASION_PILLS = [
  {val:"client dinner",label:"client dinner"},
  {val:"team drinks",label:"team drinks"},
  {val:"date night",label:"date night"},
  {val:"casual lunch",label:"casual lunch"},
  {val:"late night out",label:"late night"},
  {val:"happy hour",label:"happy hour"},
  {val:"team activity or group outing",label:"group activity"},
  {val:"private event or buyout",label:"private event"},
  {val:"celebration or birthday",label:"celebration"},
  {val:"cookies, confections, baked goods, or dessert",label:"cookies / confections"},
  {val:"takeout order for the office or a small group",label:"takeout"},
  {val:"catering for an office event, meeting, or large group",label:"catering"}
];

const VIBE_PILLS = [
  {val:"impressive, wow factor",label:"impressive / wow factor"},
  {val:"lively, fun atmosphere",label:"lively"},
  {val:"quiet enough to have a real conversation",label:"quiet to talk"},
  {val:"rooftop or outdoor seating",label:"rooftop / outdoor"},
  {val:"private room or semi-private space",label:"private room"},
  {val:"local gem, not touristy",label:"neighborhood gem"},
  {val:"celebrity chef or Michelin-recognized",label:"chef-driven"},
  {val:"unique or experiential",label:"unique / experiential"}
];

const TYPE_PILLS = [
  {val:"restaurant",label:"restaurant"},
  {val:"bar or cocktail lounge",label:"bar / lounge"},
  {val:"wine bar",label:"wine bar"},
  {val:"rooftop bar",label:"rooftop bar"},
  {val:"steakhouse",label:"steakhouse"},
  {val:"omakase or tasting menu",label:"omakase"},
  {val:"activity or experience",label:"activity / experience"},
  {val:"golf (driving range, simulator, or full course)",label:"golf"},
  {val:"live event - concert or sports game",label:"concert / sports"},
  {val:"event space or private venue buyout",label:"event space"},
  {val:"bakery, pastry shop, or confection destination",label:"bakery / confections"},
  {val:"nightclub or late night venue",label:"nightclub"},
  {val:"catering company or caterer for large groups",label:"catering"}
];

const SOURCE_PILLS = [
  {val:"Eater NY",label:"Eater NY"},
  {val:"The Infatuation",label:"The Infatuation"},
  {val:"NY Times Dining",label:"NY Times Dining"},
  {val:"Resy hot list",label:"Resy hot list"},
  {val:"Tasting Table",label:"Tasting Table"},
  {val:"Conde Nast Traveler",label:"Conde Nast"}
];
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius:8, cursor:"pointer", fontSize:13, userSelect:"none",
        border: on ? "0.5px solid #888" : "0.5px solid #ddd",
    ...
    >
      <div style={{
        ...
        border: on ? "1.5px solid #111" : "1.5px solid #ccc",
...
const inputStyle = {
  fontSize:13, padding:"7px 10px", border:"0.5px solid #ccc",
  borderRadius:8, width:"100%", background:"white", boxSizing:"border-box"
};
...
export default function App() {
  ...
  const buildPrompt = () => {
    ...
    const dateStr = dates.length
      ? dates.map(d => new Date(d + "T12:00:00").toLocaleDateString("en-US", { weekday:"long", month:"long", day:"numeric", year:"numeric" })).join(", ")
    ...
    const typeStr = type.join(", ") || "restaurant";
    const vibeStr = vibe.join(", ") || "good atmosphere";
    ...
    const deliveryNote = delivery ? "\nOnly recommend venues that offer delivery via their own service or major apps. Include which platform(s)." : "";
    const walkNote = walkIn ? "\nInclude walk-in only venues and note typical wait times." : "\nExclude walk-in only venues - user needs to book in advance.";
    ...
    const allItems = [
      ...RESTAURANTS.map(x => x.name),
      ...ACTIVITIES.map(x => x.name),
      ...VENUES.map(x => x.name + (x.good === "Yes" ? " [TEAM FAVORITE]" : ""))
    ];
    const teamCtx = useTeam === "yes" ? "\nCross-reference our team list and flag matches: " + allItems.join(", ") : "";

    return [
      "VENUE / EVENT SEARCH REQUEST - search " + allSources + " and return " + count + " recommendations.",
      "",
      ...
      "Notes: " + (notes || "none"),
      culinaryNote, deliveryNote, walkNote, resNote, teamCtx,
      "",
      "For each result include: name, neighborhood, price range, booking platform, sources that mentioned it, what the press said, why it fits, reservation/ticket tip with direct link, and availability note. Flag the top pick and note if it is on our team list. After results, include quick action buttons for more details, alternatives, or booking help."
    ].join("\n");
  };
    ...
    <div style={{ maxWidth:720, margin:"0 auto", padding:"24px 20px 60px", fontFamily:"system-ui, sans-serif" }}>
      ...
      <div style={{ display:"flex", marginBottom:24, borderBottom:"0.5px solid #e0e0e0" }}>
        {["search","team"].map(t => (
          <button key={t} onClick={() => setTab(t)} style={{
            fontSize:13, padding:"8px 16px", cursor:"pointer", background:"none", border:"none",
      ...
      {tab === "search" && (
        <div>
          ...
          <button
            onClick={() => setCulinaryEvents(prev => !prev)}
            style={{
              fontSize:13, padding:"5px 13px", borderRadius:99, cursor:"pointer", marginBottom:8,
              border: culinaryEvents ? "0.5px solid #888" : "0.5px solid #ccc",
              background: culinaryEvents ? "#f0f0f0" : "white",
              fontWeight: culinaryEvents ? 500 : 400
            }}
          >
          ...
          {culinaryEvents && (
            <p style={{ fontSize:12, color:"#888", marginBottom:16, marginTop:6 }}>
              Searching all upcoming food & wine festivals, James Beard dinners, chef popups, and immersive dining in your selected location.
            </p>
          )}
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:12, marginBottom:16 }}>
            <div>
              ...
              <input style={inputStyle} placeholder="e.g. NYC, Savannah..." value={neighborhood} onChange={e => setNeighborhood(e.target.value)} />
            </div>
            <div>
          ...
          <div style={{ display:"flex", gap:8, alignItems:"center", marginBottom:8 }}>
            ...
            <button onClick={addDate} style={{ fontSize:13, padding:"7px 14px", borderRadius:8, border:"0.5px solid #ccc", background:"white", cursor:"pointer", whiteSpace:"nowrap" }}>
              + add date
            </button>
          </div>
          <div style={{ display:"flex", flexWrap:"wrap", gap:6, marginBottom:4 }}>
            {dates.map(d => (
              <span key={d} style={{ display:"inline-flex", alignItems:"center", gap:6, fontSize:13, padding:"4px 10px", borderRadius:99, border:"0.5px solid #ccc", background:"#f5f5f5" }}>
                {fmtDate(d)}
                <button onClick={() => setDates(prev => prev.filter(x => x !== d))} style={{ background:"none", border:"none", cursor:"pointer", fontSize:14, color:"#888", padding:0, lineHeight:1 }}>x</button>
              </span>
            ))}
          </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:8 }}>
            <RadioBox on={resResy} onClick={() => setResResy(prev => !prev)}>Resy</RadioBox>
            <RadioBox on={resOT} onClick={() => setResOT(prev => !prev)}>OpenTable</RadioBox>
            <RadioBox on={resSR} onClick={() => setResSR(prev => !prev)}>SevenRooms</RadioBox>
            <RadioBox on={resTock} onClick={() => setResTock(prev => !prev)}>Tock</RadioBox>
          </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:16 }}>
            <div>
              ...
              <select style={inputStyle} value={count} onChange={e => setCount(e.target.value)}>
                {["2","3","5"].map(o => <option key={o}>{o}</option>)}
              </select>
            </div>
            <div>
          ...
          <button
            onClick={() => {
              ...
              alert("Search prompt copied! Paste it into Claude to get your recommendations.");
            }}
            style={{ width:"100%", padding:11, borderRadius:8, border:"0.5px solid #ccc", background:"#f5f5f5", fontSize:14, fontWeight:500, cursor:"pointer" }}
          >
            Find recommendations
          </button>
          <p style={{ fontSize:12, color:"#aaa", marginTop:12, textAlign:"center" }}>Copies your search to clipboard — paste into Claude to get results.</p>
        </div>
      )}
      ...
      {tab === "team" && (
        <div>
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:6, marginBottom:20 }}>
            {RESTAURANTS.map(r => (
              <span key={r.name} title={[r.desc, r.loc, r.note].filter(Boolean).join(" - ")}
                style={{ fontSize:12, padding:"4px 11px", borderRadius:99, border:"0.5px solid #ddd", background:"#fafafa" }}>
                {r.name}
              </span>
            ))}
          </div>
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:6, marginBottom:20 }}>
            {ACTIVITIES.map(a => (
              <span key={a.name} title={[a.desc, a.loc].filter(Boolean).join(" - ")}
                style={{ fontSize:12, padding:"4px 11px", borderRadius:99, border:"0.5px solid #ddd", background:"#fafafa" }}>
                {a.name}
              </span>
            ))}
            ...
            {VENUES.map(v => (
              <div key={v.name} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"8px 0", borderBottom:"0.5px solid #f0f0f0", fontSize:13, gap:8 }}>
                <span style={{ fontWeight:500 }}>
                  {v.name}
                  {v.loc && <span style={{ fontWeight:400, color:"#888" }}> - {v.loc}</span>}
                </span>
                <span style={{ display:"flex", gap:8, alignItems:"center", flexShrink:0 }}>
                  {v.min && <span style={{ color:"#888", fontSize:12 }}>{v.min}</span>}
                  <span style={{ fontSize:11, fontWeight:500, color: v.status==="Yes" ? "#1D9E75" : v.status==="No" ? "#E24B4A" : "#BA7517" }}>
                    {v.status || "-"}
                  </span>
                  {v.good === "Yes" && (
